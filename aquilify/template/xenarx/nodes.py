...
def register(keyword: str, endword: Optional[str] = None, node_class: Optional[Type] = None) -> Union[Type, Callable[[Type], Type]]:
    if node_class is not None:
        return _register(keyword, endword, node_class)
...
def _register(keyword: str, endword: Optional[str], node_class: Type) -> Type:
...
class IfNode(Node):
    ...
    re_condition = re.compile(r'''
        (not\s+)?(.+?)\s+(==|!=|<|>|<=|>=|not[ ]in|in|startswith|endswith|contains|icontains)\s+(.+)
        |
        (not\s+)?(.+)
        ''', re.VERBOSE
    )
...
class IncludeNode(Node):
    ...
    def wrender(self, context):
        ...
        if isinstance(template_name, str):
            if xenarx.loader:
                template = xenarx.loader(template_name)
                context.push()
