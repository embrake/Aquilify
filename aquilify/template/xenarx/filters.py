...
def replace(s, old, new, count=None):
    """ Replace occurrences of 'old' with 'new' in the string 's'. """
...
@register
def b64encode(value, url_safe=False):
    """ Base64 encodes the input (string, bytes, or integer). """
    if isinstance(value, int):
        value = str(value).encode('utf-8')
    elif isinstance(value, str):
        value = value.encode('utf-8')

    encoded = base64.urlsafe_b64encode(value) if url_safe else base64.b64encode(value)
